...
from navigation_tools.close_coords import close_coords as cc
...
def pipeline(mp, k, dp, He, gl):
    frame = get_frame()
    basic_cmd = movement(frame)
    self_dependence = 0
    location_coords = juet_weighted_graph["juet_coords"]
    self_dependence = cc(gl, location_coords)
    if(self_dependence == 1):
